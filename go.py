...
cmd = [
	"x11docker",
	*shlex.split(
		f"--backend=podman --network --xorg --weston-xwayland --gpu --init=none --sudouser --pulseaudio=socket --desktop --user=RETAIN --xtest --workdir={user_dir} --cap-default --newprivileges=yes"
	),
        '--',
        *PODMAN_OPTIONS,
        '--',
